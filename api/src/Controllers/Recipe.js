...
const getRecipes = async (req, res) => {
    ...
    if(!name){
        ...
        const recipeName = await allRecipes.filter( e => e.name.toLowerCase().includes(name.toLowerCase()));
        recipeName.length > 0 ?
...
const createRecipe = async (req, res) => {
    const {
        name, summary, score, healthScore, instructions, dietTypes, createdInDb
    } = req.body;

    const recipeCreated = await Recipe.create({
        name, summary, score, healthScore, instructions, createdInDb
    })
